...
static void lb_draw(void) {
    ...
    lb_center("HIGH SCORES", assets.messageFont, sys.winHeight * 0.06f,
              Fade((Color){ 255, 215, 70, 255 }, lb_bg));

    char best[64];
    snprintf(best, sizeof(best), "YOUR BEST: %d", sys.highScore);
    lb_center(best, assets.screenFont, sys.winHeight * 0.24f, Fade((Color){ 180, 180, 180, 255 }, lb_bg));

    const LBBoard *board = Leaderboard_FindBoard(&g_lb, LB_SCORE_METRIC);

    if (g_lb.fetch_state == LB_PENDING) {
        lb_center("LOADING...", assets.screenFont, sys.winHeight * 0.5f, Fade(WHITE, lb_bg));
        ...
        const char *msg = (g_lb.net_status == LB_NET_OFFLINE)
                        ...
                        : "NO CONNECTION";
        lb_center(msg, assets.screenFont, sys.winHeight * 0.5f, Fade((Color){ 160, 160, 160, 255 }, lb_bg));
    } else {
        ...
        for (int i = 0; i < board->count; i++) {
            ...
            DrawTextEx(assets.screenFont, rank, (Vector2){ x, y }, fs, 2.0f, Fade(WHITE, lb_bg));
            ...
            DrawTextEx(assets.screenFont, val, (Vector2){ xs - vm.x, y }, fs, 2.0f,
                       Fade((Color){ 90, 230, 200, 255 }, lb_bg));
            y += rh;
        }
    }
