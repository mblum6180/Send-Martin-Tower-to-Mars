...
void UpdatePhysics(void)
{
    ...
    // Clamp the catch-up (local edit). A long gap since the previous call - e.g.
    // a level that doesn't step physics, like this game's Level 2 - or a very
    // slow frame must not queue thousands of substeps and teleport bodies. Cap at
    // a few frames' worth (~30 steps at the 1.667ms step).
    if (deltaTimeAccumulator > 50.0) deltaTimeAccumulator = 50.0;
