...
// Desktop (OpenGL 3.3 core) port of the lightly-salted-engine CRT shader
// (assets/shaders/crt.web.fs is the #version 100 GLSL-ES original for web).
// Uniform names must match engine/crt.c's GetShaderLocation calls.
...
vec3 bloomTap(vec2 uv) {
    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0)
        return vec3(0.0);
    return thresholdGlow(texture(texture0, uv).rgb);
}
...
void main() {
    ...
    vec4 baseSample = texture(texture0, distortedUv);
    ...
    if (u_persistenceStrength > 0.0) {
        vec3 previousGlow = thresholdGlow(texture(u_previousFrame, distortedUv).rgb);
    ...
    vec3 separated = vec3(
        texture(texture0, clamp(distortedUv + rgbOffset, 0.0, 1.0)).r,
        texture(texture0, distortedUv).g,
        texture(texture0, clamp(distortedUv - rgbOffset, 0.0, 1.0)).b
    );
    ...
    outColor = vec4(clamp(finalColor, 0.0, 1.0), baseSample.a);
}
